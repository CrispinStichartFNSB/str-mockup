...
const currentUnits = ref<UnitSelection[]>([])
...
<template>
  <div class="lab-shell">
    ...
    <main class="lab-layout">
      <aside class="lab-sidebar" aria-labelledby="address-lab-controls-heading">
        <p class="eyebrow">Component test page</p>
        <h1 id="address-lab-controls-heading">Address-First Lab</h1>
        <p class="lab-intro">Change the component props and inspect its emitted state.</p>

        <fieldset class="lab-control-group">
          <legend>State</legend>
          <label class="toggle-control" for="address-component-disabled">
            <input id="address-component-disabled" v-model="disabled" type="checkbox" />
            <span>
        ...
        <fieldset class="lab-control-group">
          ...
          <label
            ...
            :for="`address-first-prop-${address.id}`"
          >
            <input
              :id="`address-first-prop-${address.id}`"
        ...
        <section class="lab-output" aria-labelledby="address-component-output-heading">
          <h2 id="address-component-output-heading">Component output</h2>
          <dl>
            <div>
              <dt>Valid</dt>
              <dd id="address-lab-output-validity" aria-live="polite">
                {{ isValid ? 'true' : 'false' }}
              </dd>
            </div>
            <div>
              <dt>Units</dt>
              <dd id="address-lab-output-count" aria-live="polite">
                {{ currentUnits.length }}
              </dd>
            </div>
          </dl>
          <pre aria-label="Emitted address-first units">{{
            JSON.stringify(currentUnits, null, 2)
          }}</pre>
        </section>
      ...
      <section class="lab-preview" aria-labelledby="address-component-preview-heading">
        <header class="lab-preview-heading">
          <div>
            <p class="eyebrow">Live preview</p>
            <h2 id="address-component-preview-heading">Units by address</h2>
          </div>
          <code>{{ addresses.length }} addresses · disabled: {{ disabled }}</code>
        </header>

        <div class="component-frame">
          <AddressUnitSelection
            :key="componentKey"
            :addresses="addresses"
            :disabled="disabled"
